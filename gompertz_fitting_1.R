...
plotMortalityParameters <- function(resM,regrStart=50,lastPerStart=2007,savePng=0) {
    ...
    par(mfrow=c(1,3))
    plot(yrs,a0M,col=4,ylim=c(-12.5,-9.5),xlab="Year",ylab="Basic mortality",main="Parameter b in y=b+ax")
    ...
    plot(resM[,1],resM[,3],col=4,ylim=c(0.09,0.12),xlab="Year",ylab="Aging mortality component",main="Parameter a in y=b+ax")
    ...
    plot(a0M,resM[,3],col=4,xlim=c(-12.5,-9.5),ylim=c(0.09,0.12),xlab="b-parameter",ylab="a-parameter",main="a vs b parameters in y=b+ax")
...
testMakehamHazard <- function(lambda0,lambdaStep=1.1,a=-11.5,b=0.105) {
    ...
    lambda <-  0 ;
    X11() ;
    plot(ages,logHazard(ages,lambda),type="l",lwd=2)
    lambda <- lambda0 ;
    for (i in 1:nSteps)  {
        lHz <-logHazard(ages,lambda) ; 
        points(ages,lHz,type="l",lty=4,col=4) ; lambda <- lambda*lambdaStep ;
    }    
...
# save(sT,file="../data/survival_1966-2017.RData")
# load("../data/survival_1966-2017.RData")

#> pG <- processingGompertz()
#> rC <- pG$regrC
#> plotPopHistograms(pG$regrC)
#> plotHistHistograms(pG$resM)
